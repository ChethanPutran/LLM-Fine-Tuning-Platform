...
import logging
from backend.common.job_models import BaseJob, JobPriority, JobStatus
...
class BaseController(ABC):
    ...
    async def execute_job(self, job_id: str) -> Dict[str, Any]:
        ...
        Returns a payload that satisfies ExecutionStatusResponse.
        """
        try:
        ...
        try:
            ...
            logger.exception("orchestrator.execute_job failed for %s", job_id)
            return {
                "job_id": str(job_uuid),
                "execution_id": None,
                "status": "error",
        ...
        # Normalize whatever the orchestrator returned. The critical line:
        # `execution_id` may be None — never `str(...)` it, or you get 'None'.
        exec_id = execution_result.get("execution_id")
        if exec_id in (None, "", "None", "null"):
            exec_id = None

        if execution_result.get("status") == "error" or execution_result.get("error"):
            return {
                "job_id": str(job_uuid),
