...
from backend.dependencies.controller import get_data_collection_controller
from backend.controllers.data_collection_controller import DataCollectionController
from backend.api.models import ExecutionStatusResponse, JobCreationResponse, JobStatusResponse, ListJobsResponse, ListResourcesResponse, RequestBase, StartCollectionRequest, StatisticsResponse 
...
async def execute_collection_job(
    ...
    try:
        ...
        # Controller now always returns {"status": ...}
        if result.get("status") == "error" or result.get("error"):
            raise HTTPException(
                status_code=400,
                detail=result.get("error", "Failed to execute job"),
            )
