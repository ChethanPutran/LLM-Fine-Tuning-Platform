# backend/core/pipeline_engine/orchestrator.py

import asyncio
import logging
from typing import Dict, List, Optional, Any
from uuid import UUID, uuid4
from datetime import datetime, timezone

from backend.core.pipeline_engine.builder import PipelineBuilder
from backend.common.enums import ExecutionEvent, NodeType
from backend.core.pipeline_engine.retry_handler import RetryHandler
from backend.core.pipeline_engine.models import Pipeline
from backend.core.pipeline_engine.executor import PipelineExecutor
from backend.core.pipeline_engine.scheduler import PipelineScheduler, PriorityScheduler
from backend.core.pipeline_engine.dag_validator import DAGValidator
from backend.core.pipeline_engine.optimizer import PipelineOptimizer
from backend.core.pipeline_engine.code_generator import PipelineCodeGenerator
from backend.core.pipeline_engine.state_manager import PipelineStateManager, RedisStateStorage
from backend.core.execution.resource_manager import ResourceManager
from backend.core.execution.async_executor import AsyncExecutor
from backend.common.job_models import JobPriority, BaseJob
from backend.api.websocket import manager

logger = logging.getLogger(__name__)


def _to_uuid(value) -> UUID:
    """Normalize a UUID or str into a UUID instance."""
    if isinstance(value, UUID):
        return value
    return UUID(str(value))


class ExecutionContext:
    """Tracks state of a single pipeline execution."""

    def __init__(
        self,
        execution_id: UUID,
        pipeline: Pipeline,
        user_id: Optional[str],
        priority: JobPriority,
        start_time: datetime,
    ):
        self.execution_id = execution_id
        self.pipeline = pipeline
        self.user_id = user_id
        self.priority = priority
        self.start_time = start_time
        self.end_time: Optional[datetime] = None
        self.status: str = "created"
        self.error: Optional[str] = None
        self.optimization_summary: Dict[str, Any] = {}
        self.execution_plan: List[List[str]] = []
        self.generated_code_path: Optional[str] = None
        self.completed_nodes: List[str] = []
        self.failed_nodes: List[str] = []
        self.node_results: Dict[str, Any] = {}
        self.errors: Dict[str, str] = {}
        self.retry_counts: Dict[str, int] = {}
        self.result: Optional[Dict] = None


class PipelineOrchestrator:
    """
    Main orchestrator that connects all components.
    Single source of truth for pipeline execution.
    """

    def __init__(self, num_workers: int = 4):
        self.redis = RedisStateStorage()
        self.state_manager = PipelineStateManager(self.redis)
        self.scheduler = PipelineScheduler(PriorityScheduler())
        self.resource_manager = ResourceManager()
        self.retry_handler = RetryHandler()
        self.pipeline_optimizer = PipelineOptimizer()
        self.code_generator = PipelineCodeGenerator()

        self.pipeline_builder = PipelineBuilder(
            name="Global Pipeline",
            description="Pipeline builder for user-defined pipelines",
        )

        self.pipeline_executor = PipelineExecutor(
            state_manager=self.state_manager,
            scheduler=self.scheduler,
            retry_handler=self.retry_handler,
        )

        self.async_executor = AsyncExecutor(
            self.pipeline_executor, self.resource_manager, num_workers
        )

        self.active_executions: Dict[UUID, ExecutionContext] = {}
        self._jobs: Dict[UUID, BaseJob] = {}
        self._job_metadata: Dict[UUID, Dict[str, Any]] = {}
        self.active_jobs: Dict[UUID, BaseJob] = {}

        self._register_all_handlers()
        self._setup_event_handlers()

    # ------------------------------------------------------------------
    # Lookups
    # ------------------------------------------------------------------

    def get_active_jobs(self) -> List[Dict[str, Any]]:
        return [job.model_dump() for job in self.active_jobs.values()]

    async def get_queued_jobs(self) -> List[Dict[str, Any]]:
        return await self.async_executor.get_queued_jobs()

    # ------------------------------------------------------------------
    # Handler registration
    # ------------------------------------------------------------------

    def _register_all_handlers(self):
        from backend.core.pipeline_engine.handlers.data_collection_handler import DataCollectionHandler
        from backend.core.pipeline_engine.handlers.preprocessing_handler import PreprocessingHandler
        from backend.core.pipeline_engine.handlers.training_handler import TrainingHandler
        from backend.core.pipeline_engine.handlers.finetuning_handler import FinetuningHandler
        from backend.core.pipeline_engine.handlers.optimization_handler import OptimizationHandler
        from backend.core.pipeline_engine.handlers.deployment_handler import DeploymentHandler
        from backend.core.pipeline_engine.handlers.tokenization_handler import TokenizationHandler
        from backend.core.pipeline_engine.handlers.evaluation_handler import EvaluationHandler

        self.handlers = {
            NodeType.DATA_INGESTION.value: DataCollectionHandler(),
            NodeType.DATA_PROCESSING.value: PreprocessingHandler(),
            NodeType.MODEL_TRAINING.value: TrainingHandler(),
            NodeType.MODEL_FINETUNING.value: FinetuningHandler(),
            NodeType.MODEL_EVALUATION.value: EvaluationHandler(),
            NodeType.OPTIMIZATION.value: OptimizationHandler(),
            NodeType.MODEL_DEPLOYMENT.value: DeploymentHandler(),
            NodeType.TOKENIZATION.value: TokenizationHandler(),
        }

        worker_handler_aliases = {
            "data_collection": self.handlers[NodeType.DATA_INGESTION.value],
            "data_processing": self.handlers[NodeType.DATA_PROCESSING.value],
            "training": self.handlers[NodeType.MODEL_TRAINING.value],
            "finetuning": self.handlers[NodeType.MODEL_FINETUNING.value],
            "evaluation": self.handlers[NodeType.MODEL_EVALUATION.value],
            "optimization": self.handlers[NodeType.OPTIMIZATION.value],
            "deployment": self.handlers[NodeType.MODEL_DEPLOYMENT.value],
            "tokenization": self.handlers[NodeType.TOKENIZATION.value],
        }

        for node_type, handler in self.handlers.items():
            self.pipeline_executor.register_node_handler(node_type, handler.execute)
        for job_type, handler in worker_handler_aliases.items():
            for worker in self.async_executor.workers:
                worker.register_handler(job_type, handler.execute)

        logger.info("Registered %d job handlers", len(self.handlers))

    # ------------------------------------------------------------------
    # Validation
    # ------------------------------------------------------------------

    async def validate_pipeline(self, pipeline_json: Dict[str, Any]) -> bool:
        validator = await DAGValidator.from_dict(pipeline_json)
        is_valid, errors = validator.validate()
        if not is_valid:
            raise ValueError(f"Invalid pipeline definition: {errors}")
        return is_valid

    # ------------------------------------------------------------------
    # Event handlers
    # ------------------------------------------------------------------

    def _setup_event_handlers(self):

        @self.pipeline_executor.on(ExecutionEvent.NODE_STARTED)
        async def on_node_started(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            node_id = data.get("node_id")
            job_id = data.get("job_id")
            node_name = data.get("node_name")
            node_type = data.get("node_type")

            if execution_id is None or node_id is None:
                logger.error("Invalid NODE_STARTED event: missing execution_id or node_id")
                return

            execution_id = _to_uuid(execution_id)
            node_id_str = str(node_id)

            logger.info(
                "Node %s (%s) started for execution %s",
                node_name, node_type, execution_id,
            )

            await self._emit_execution_update(
                {
                    "execution_id": str(execution_id),
                    "node_id": node_id_str,
                    "node_status": "running",
                    "progress": 0,
                    "message": f"Node {node_name or node_id_str} started",
                    "level": "info",
                },
                job_id=job_id,
            )

        @self.pipeline_executor.on(ExecutionEvent.NODE_COMPLETED)
        async def on_node_completed(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            node_id = data.get("node_id")
            job_id = data.get("job_id")
            result = data.get("result")

            if execution_id is None or node_id is None:
                logger.error("Invalid NODE_COMPLETED event: missing execution_id or node_id")
                return

            execution_id = _to_uuid(execution_id)
            node_id_str = str(node_id)

            logger.info("Node %s completed for execution %s", node_id_str, execution_id)

            if execution_id in self.active_executions:
                ctx = self.active_executions[execution_id]
                if node_id_str not in ctx.completed_nodes:
                    ctx.completed_nodes.append(node_id_str)
                ctx.node_results[node_id_str] = result

                if len(ctx.completed_nodes) == len(ctx.pipeline.nodes):
                    ctx.status = "completed"
                    ctx.end_time = datetime.now(timezone.utc)
                    await self._notify_execution_complete(execution_id, ctx)
            else:
                logger.warning("Execution %s not found in active executions", execution_id)

            await self._emit_execution_update(
                {
                    "execution_id": str(execution_id),
                    "node_id": node_id_str,
                    "node_status": "completed",
                    "progress": 100,
                    "result": result,
                    "message": f"Node {node_id_str} completed",
                    "level": "success",
                },
                job_id=job_id,
            )

        @self.pipeline_executor.on(ExecutionEvent.NODE_FAILED)
        async def on_node_failed(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            node_id = data.get("node_id")
            job_id = data.get("job_id")
            error = data.get("error")

            if execution_id is None or node_id is None:
                logger.error("Invalid NODE_FAILED event: missing execution_id or node_id")
                return

            execution_id = _to_uuid(execution_id)
            node_id_str = str(node_id)

            logger.error(
                "Node %s failed for execution %s: %s",
                node_id_str, execution_id, error,
            )

            ctx = self.active_executions.get(execution_id)
            if ctx is not None:
                if node_id_str not in ctx.failed_nodes:
                    ctx.failed_nodes.append(node_id_str)
                ctx.errors[node_id_str] = str(error)
                ctx.status = "failed"
                ctx.end_time = datetime.now(timezone.utc)
                await self._notify_execution_failed(execution_id, ctx)

            await self._emit_execution_update(
                {
                    "execution_id": str(execution_id),
                    "node_id": node_id_str,
                    "node_status": "failed",
                    "progress": 0,
                    "error": str(error),
                    "message": f"Node {node_id_str} failed: {error}",
                    "level": "error",
                },
                job_id=job_id,
            )

        @self.pipeline_executor.on(ExecutionEvent.PIPELINE_STARTED)
        async def on_pipeline_started(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            pipeline_name = data.get("pipeline_name")

            if execution_id is None:
                logger.error("Invalid PIPELINE_STARTED event: missing execution_id")
                return

            execution_id = _to_uuid(execution_id)
            logger.info("Pipeline %s started (execution: %s)", pipeline_name, execution_id)

            if execution_id in self.active_executions:
                self.active_executions[execution_id].status = "running"
                self.active_executions[execution_id].start_time = datetime.now(timezone.utc)

            await self._emit_execution_update({
                "execution_id": str(execution_id),
                "pipeline_status": "running",
                "pipeline_name": pipeline_name,
                "message": f"Pipeline {pipeline_name} started",
                "level": "info",
            })

        @self.pipeline_executor.on(ExecutionEvent.PIPELINE_COMPLETED)
        async def on_pipeline_completed(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            status = data.get("status", "completed")

            if execution_id is None:
                logger.error("Invalid PIPELINE_COMPLETED event: missing execution_id")
                return

            execution_id_str = _to_uuid(execution_id)
            logger.info("Pipeline %s completed with status: %s", execution_id_str, status)

            if execution_id_str in self.active_executions:
                self.active_executions[execution_id_str].status = status
                self.active_executions[execution_id_str].end_time = datetime.now(timezone.utc)

            await self._emit_execution_update({
                "execution_id": str(execution_id_str),
                "pipeline_status": status,
                "message": f"Pipeline completed with status: {status}",
                "level": "success" if status == "completed" else "error",
            })

        @self.pipeline_executor.on(ExecutionEvent.PIPELINE_FAILED)
        async def on_pipeline_failed(data: Dict[str, Any]):
            execution_id = data.get("execution_id")
            error = data.get("error")

            if execution_id is None:
                logger.error("Invalid PIPELINE_FAILED event: missing execution_id")
                return

            execution_id_str = _to_uuid(execution_id)
            logger.error("Pipeline %s failed: %s", execution_id_str, error)

            if execution_id_str in self.active_executions:
                self.active_executions[execution_id_str].status = "failed"
                self.active_executions[execution_id_str].end_time = datetime.now(timezone.utc)
                self.active_executions[execution_id_str].error = str(error)

            await self._emit_execution_update({
                "execution_id": str(execution_id_str),
                "pipeline_status": "failed",
                "error": str(error),
                "message": f"Pipeline failed: {error}",
                "level": "error",
            })

    # ==================== Job Management ====================

    async def execute_job(self, job_id: UUID) -> Dict[str, Any]:
        """Execute a single registered job."""
        job = self._jobs.get(job_id)
        if not job:
            logger.error("Job %s not found for execution", job_id)
            return {
                "job_id": str(job_id),
                "execution_id": None,
                "status": "error",
                "error": f"Job {job_id} not found",
            }

        try:
            pipeline = self._build_single_job_pipeline(job)

            result = await self._execute_pipeline(
                pipeline,
                user_id=job.user_id,
                priority=job.priority,
            )

            execution_id = result.get("execution_id")

            try:
                job.execution_id = UUID(str(execution_id)) if execution_id else None
            except (ValueError, TypeError, AttributeError):
                logger.warning("Could not store execution_id on job %s", job_id)

            return {
                "job_id": str(job_id),
                "execution_id": str(execution_id) if execution_id else None,
                "status": result.get("status", "started"),
                "message": "Job execution started successfully",
            }

        except Exception as e:
            logger.exception("Failed to execute job %s", job_id)
            return {
                "job_id": str(job_id),
                "execution_id": None,
                "status": "error",
                "error": str(e),
            }

    def _build_single_job_pipeline(self, job: BaseJob) -> Pipeline:
        metadata = self._job_metadata.get(job.job_id)
        if not metadata:
            raise ValueError(
                f"No registered metadata for job {job.job_id}; "
                "was register_job called for this job?"
            )

        builder = PipelineBuilder(
            name=f"{job.job_type.value}-{job.job_id}",
            description=f"Single-job pipeline for {job.job_id}",
        )
        builder.add_node(
            name=metadata["name"],
            node_type=metadata["node_type"],
            resources=metadata["resources"],
            retry_policy=metadata["retry_policy"],
            metadata=metadata["metadata"],
            position=(0, 0),
            job=job,
        )
        return builder.build()

    def register_job(self, job: BaseJob, metadata: Dict[str, Any]) -> UUID:
        """Register a job in the orchestrator."""
        # Ensure the job_id is available to event handlers.
        metadata.setdefault("metadata", {})
        metadata["metadata"]["job_id"] = str(job.job_id)

        self.add_node_to_pipeline(
            name=metadata["name"],
            node_type=metadata["node_type"],
            resources=metadata["resources"],
            metadata=metadata["metadata"],
            retry_policy=metadata["retry_policy"],
            position=metadata["position"],
            job=job,
        )
        self._jobs[job.job_id] = job
        self._job_metadata[job.job_id] = metadata
        logger.info("Job %s registered with type %s", job.job_id, job.job_type.value)
        return job.job_id

    async def remove_job(self, job_id: UUID) -> bool:
        job = self._jobs.get(job_id)
        if not job:
            logger.error("Job %s not found for removal", job_id)
            return False
        if job.execution_id:
            await self.cancel_execution(job.execution_id)
        job.mark_removed()
        await manager.notify_job_update(str(job_id), {
            "status": "removed",
            "message": "Job removed by user",
        })
        return True

    def get_job(self, job_id: UUID) -> Optional[BaseJob]:
        return self._jobs.get(job_id)

    def update_job(self, job_id: UUID, **kwargs) -> bool:
        job = self._jobs.get(job_id)
        if job:
            for key, value in kwargs.items():
                if hasattr(job, key):
                    setattr(job, key, value)
            job.updated_at = datetime.now(timezone.utc)
            return True
        return False

    def list_jobs(
        self,
        job_type: Optional[str] = None,
        status: Optional[str] = None,
        user_id: Optional[str] = None,
        limit: int = 50,
        offset: int = 0,
    ) -> Dict[str, Any]:
        jobs_list = list(self._jobs.values())
        jobs_list.sort(key=lambda x: x.created_at, reverse=True)

        if job_type:
            jobs_list = [j for j in jobs_list if j.job_type.value == job_type]
        if status:
            jobs_list = [j for j in jobs_list if j.status.value == status]
        if user_id:
            jobs_list = [j for j in jobs_list if j.user_id == user_id]

        total = len(jobs_list)
        paginated_jobs = jobs_list[offset:offset + limit]

        return {
            "total": total,
            "limit": limit,
            "offset": offset,
            "jobs": paginated_jobs,
        }

    # ==================== Pipeline Builder ====================

    def add_node_to_pipeline(
        self,
        name: str,
        node_type: NodeType,
        resources: Dict,
        retry_policy: Dict,
        metadata: Dict,
        position: tuple,
        job: Optional[Any] = None,
    ) -> "PipelineOrchestrator":
        self.pipeline_builder.add_node(
            name=name,
            node_type=node_type,
            resources=resources,
            retry_policy=retry_policy or {"max_retries": 3, "strategy": "exponential"},
            metadata=metadata,
            position=position,
            job=job,
        )
        return self

    def add_edge_to_pipeline(
        self, source: str, target: str, condition: str, label: str
    ) -> "PipelineOrchestrator":
        self.pipeline_builder.add_edge(source, target, condition, label)
        return self

    def clear_pipeline(self) -> "PipelineOrchestrator":
        return self

    def get_pipeline(self) -> Pipeline:
        return self.pipeline_builder.build()

    # ==================== Execution ====================

    async def execute_current_pipeline(
        self,
        user_id: Optional[str] = None,
        priority: JobPriority = JobPriority.NORMAL,
    ) -> Dict[str, Any]:
        pipeline = self.pipeline_builder.build()
        return await self._execute_pipeline(pipeline, user_id, priority)

    async def _execute_pipeline(
        self,
        pipeline: Pipeline,
        user_id: Optional[str] = None,
        priority: JobPriority = JobPriority.NORMAL,
    ) -> Dict[str, Any]:
        optimization_result = self.pipeline_optimizer.optimize(pipeline)
        pipeline = optimization_result.pipeline
        execution_id = uuid4()

        generated_code = self.code_generator.generate(
            pipeline=pipeline,
            execution_plan=optimization_result.execution_plan,
            optimization_summary=optimization_result.summary,
            execution_id=execution_id,
            user_id=user_id,
            priority=priority,
        )

        execution_context = ExecutionContext(
            execution_id=execution_id,
            pipeline=pipeline,
            user_id=user_id,
            priority=priority,
            start_time=datetime.now(timezone.utc),
        )
        execution_context.optimization_summary = optimization_result.summary
        execution_context.execution_plan = optimization_result.execution_plan
        execution_context.generated_code_path = generated_code["path"]
        self.active_executions[execution_id] = execution_context

        logger.info(
            "Created optimized execution %s for pipeline %s: %s",
            execution_id,
            pipeline.name,
            optimization_result.summary,
        )

        await self.state_manager.save_state(execution_id, pipeline)

        asyncio.create_task(
            self._execute_pipeline_async(execution_id, pipeline, execution_context)
        )

        return {
            "execution_id": execution_id,
            "pipeline_id": pipeline.id,
            "pipeline_name": pipeline.name,
            "status": "started",
            "optimization": optimization_result.summary,
            "execution_plan": optimization_result.execution_plan,
            "generated_code_path": generated_code["path"],
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }

    async def _execute_pipeline_async(
        self,
        execution_id: UUID,
        pipeline: Pipeline,
        context: ExecutionContext,
    ):
        try:
            context.status = "running"

            await self._emit_execution_update({
                "execution_id": str(execution_id),
                "pipeline_status": "running",
                "pipeline_name": pipeline.name,
                "optimization": context.optimization_summary,
                "execution_plan": context.execution_plan,
                "generated_code_path": context.generated_code_path,
                "message": "Execution started",
                "level": "info",
            })

            result = await self.pipeline_executor.execute_pipeline(
                pipeline=pipeline,
                execution_id=execution_id,
                resource_availability=await self.resource_manager.get_available_resources(),
            )

            context.status = result["status"]
            context.end_time = datetime.now(timezone.utc)
            context.result = result

            await self.state_manager.save_state(execution_id, pipeline)

            await self._emit_execution_update({
                "execution_id": str(execution_id),
                "pipeline_status": result["status"],
                "optimization": context.optimization_summary,
                "execution_plan": context.execution_plan,
                "generated_code_path": context.generated_code_path,
                "duration": (context.end_time - context.start_time).total_seconds(),
                "message": f"Execution completed with status {result['status']}",
                "level": "success" if result["status"] == "completed" else "error",
            })

            logger.info(
                "Execution %s completed with status: %s",
                execution_id, result["status"],
            )

        except Exception as e:
            logger.exception("Execution %s failed: %s", execution_id, e)
            context.status = "failed"
            context.error = str(e)

            for job_id, job in self._jobs.items():
                if getattr(job, "execution_id", None) == execution_id:
                    job.mark_failed(str(e))
                    await manager.notify_job_update(str(job_id), {
                        "status": "failed",
                        "error": str(e),
                    })
                    break

            await self._emit_execution_update({
                "execution_id": str(execution_id),
                "pipeline_status": "failed",
                "error": str(e),
                "message": f"Execution failed: {e}",
                "level": "error",
            })

        finally:
            await self.state_manager.cleanup_checkpoints(execution_id)

    async def get_execution_status(self, execution_id: UUID) -> Dict[str, Any]:
        if execution_id not in self.active_executions:
            pipeline = await self.state_manager.load_state(execution_id)
            if pipeline:
                return {
                    "execution_id": execution_id,
                    "status": "completed",
                    "pipeline": pipeline.name,
                }
            return {"error": "Execution not found"}

        context = self.active_executions[execution_id]
        return {
            "execution_id": execution_id,
            "status": context.status,
            "pipeline_name": context.pipeline.name,
            "completed_nodes": len(context.completed_nodes),
            "failed_nodes": len(context.failed_nodes),
            "total_nodes": len(context.pipeline.nodes),
            "progress": len(context.completed_nodes) / len(context.pipeline.nodes) * 100
            if context.pipeline.nodes else 0,
            "optimization": context.optimization_summary,
            "execution_plan": context.execution_plan,
            "generated_code_path": context.generated_code_path,
            "start_time": context.start_time.isoformat(),
            "end_time": context.end_time.isoformat() if context.end_time else None,
            "node_results": context.node_results,
            "errors": context.errors,
        }

    async def cancel_execution(self, execution_id: UUID) -> bool:
        if execution_id in self.active_executions:
            context = self.active_executions[execution_id]
            if context.status == "running":
                cancelled = self.pipeline_executor.cancel_pipeline(execution_id)
                if cancelled:
                    context.status = "cancelled"
                    for job_id, job in self._jobs.items():
                        if getattr(job, "execution_id", None) == execution_id:
                            job.mark_cancelled()
                            await manager.notify_job_update(str(job_id), {
                                "status": "cancelled",
                                "message": "Execution cancelled by user",
                            })
                            break
                    logger.info("Execution %s cancelled", execution_id)
                return cancelled
        return False

    # ==================== Logs ====================

    async def get_execution_logs(
        self, execution_id: UUID, node_id: str = "*"
    ) -> Optional[List[str]]:
        return await self.state_manager.get_logs(execution_id)

    # ==================== Notifications ====================

    async def _emit_execution_update(
        self,
        data: Dict[str, Any],
        *,
        job_id: Optional[str] = None,
    ) -> None:
        """
        Push a single execution_update frame to every connected WS client.

        `job_id` is attached to the payload when provided so the frontend
        can match the update back to a specific stage (stages are keyed by
        jobId in the UI, not by the backend's ephemeral node id).
        """
        execution_id = data.get("execution_id")
        if not execution_id:
            logger.warning("_emit_execution_update called without execution_id")
            return

        payload = {
            "timestamp": data.get("timestamp") or datetime.now(timezone.utc).isoformat(),
            "level": data.get("level", "info"),
            **data,
        }
        if job_id:
            payload["job_id"] = str(job_id)

        envelope = {
            "type": "execution_update",
            "execution_id": str(execution_id),
            "data": payload,
        }

        logger.debug("Emitting execution update: %s", envelope)

        try:
            await manager.broadcast_to_all_clients(envelope)
        except Exception as e:
            logger.error("Failed to broadcast execution update: %s", e)

    async def _broadcast_status(self, status_update: Dict[str, Any]) -> None:
        """
        Legacy generic broadcaster. Kept for non-execution payloads
        (e.g. ad-hoc diagnostics). For execution updates prefer
        `_emit_execution_update` so the payload shape matches what the
        frontend expects.
        """
        logger.debug("Status update: %s", status_update)
        try:
            await manager.broadcast_to_all_clients(status_update)
        except Exception as e:
            logger.error("Failed to broadcast status update: %s", e)

    async def _notify_execution_complete(self, execution_id: UUID, context: ExecutionContext):
        logger.info(
            "Execution %s complete: %d nodes succeeded",
            execution_id, len(context.completed_nodes),
        )

    async def _notify_execution_failed(self, execution_id: UUID, context: ExecutionContext):
        logger.error(
            "Execution %s failed: %d nodes failed",
            execution_id, len(context.failed_nodes),
        )

    async def _calculate_average_duration(self, user_id: Optional[str] = None) -> float:
        durations = []
        for ctx in self.active_executions.values():
            if ctx.status == "completed" and (user_id is None or ctx.user_id == user_id):
                if ctx.end_time and ctx.start_time:
                    durations.append((ctx.end_time - ctx.start_time).total_seconds())
        return sum(durations) / len(durations) if durations else 0.0

    # ==================== Statistics ====================

    async def get_statistics(
        self, job_type: Optional[str] = None, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        return {
            "total_jobs": len(self._jobs),
            "in_progress_jobs": sum(
                1 for job in self.active_jobs.values() if job.status == "in_progress"
            ),
            "completed_jobs": sum(
                1 for ctx in self.active_executions.values() if ctx.status == "completed"
            ),
            "failed_jobs": sum(
                1 for ctx in self.active_executions.values() if ctx.status == "failed"
            ),
            "cancelled_jobs": sum(
                1 for ctx in self.active_executions.values() if ctx.status == "cancelled"
            ),
            "removed_jobs": sum(
                1 for ctx in self.active_executions.values() if ctx.status == "removed"
            ),
            "average_duration": await self._calculate_average_duration(user_id),
            "status": "success",
            "message": "Statistics retrieved successfully",
            "error": None,
            "tags": [],
        }

    # ==================== Lifecycle ====================

    async def start(self):
        await self.async_executor.start()
        logger.info("Pipeline orchestrator started")

    async def stop(self):
        await self.async_executor.stop()
        logger.info("Pipeline orchestrator stopped")