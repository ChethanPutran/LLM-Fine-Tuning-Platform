# backend/src/backend/core/training/config.py
...
class TrainingParameters:
    ...
    # ↓ dataclasses.field, NOT pydantic.Field
...
class TrainingConfig(BaseModel):
    ...
    learning_rate: float = Field(
        default=5e-5, ge=1e-6, le=1e-3, description="Learning rate for training"
    )
    num_epochs: int = Field(
        default=3, ge=1, le=100, description="Number of epochs for training"
    )
    max_length: int = Field(
        default=512, ge=128, le=4096, description="Maximum sequence length for training"
    )
    additional_params: TrainingParameters = Field(
        default_factory=TrainingParameters,
        description="Additional parameters for training",
    )
    output_model_path: Optional[str] = None
    task: TrainingTaskConfig = Field(
        default_factory=lambda: TrainingTaskConfig(task_name="text_classification"),
        description="Configuration for the training task",
    )