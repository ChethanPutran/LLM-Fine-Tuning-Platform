...
from backend.old.model.Transformers.pre_processing import PositionalEncoding, Embedding, generate_vocaculary, get_one_hot


class Attention(nn.Module):
    def __init__(self, emd_size, max_len, word_vecs: torch.Tensor = None):
        super().__init__()
        self.K = torch.clone(word_vecs) if word_vecs else torch.rand(
            (max_len, emd_size))
        self.Q = torch.clone(word_vecs) if word_vecs else torch.rand(
            (max_len, emd_size))
        self.V = torch.clone(word_vecs) if word_vecs else torch.rand(
            (max_len, emd_size))
